...
class Gallery extends Component {
  ...
  onClickChange = (imageUrl, imageAltText) => {
    this.setState({DisplayImg: imageUrl, altText: imageAltText})
  }

  render() {
    const {DisplayImg, altText} = this.state

    return (
      <div className="bgContainer">
        <div className="contentContainer">
          <img src={DisplayImg} className="img-style" alt={altText} />
          ...
          <ul className="thumbNailContainer">
            {imagesList.map(eachItem => (
              <ThumbnailItem
                thumbnailList={eachItem}
                key={eachItem.id}
                isActive={eachItem.imageAltText === altText}
